...
export class AutoAcceptService {
    ...
    constructor(interval: number = 800, config?: AutoAcceptConfig) {
        ...
        if (config?.acceptKeywords || config?.rejectKeywords) {
            this._cdp.updateKeywords(
                ...
                config?.rejectKeywords ?? ['skip', 'reject', 'cancel', 'close', 'refine', 'always', 'aga:']
            );
        }
    }

    start(): void {
        logInfo(`start() called. Current _enabled state is: ${this._enabled}`);
    ...
    stop(): void {
        logInfo(`stop() called. Current _enabled state is: ${this._enabled}`);
        if (!this._enabled) return;
        this._enabled = false;
        if (this._timer) {
    ...
    toggle(): boolean {
        logInfo(`toggle() called. Current _enabled state is: ${this._enabled}`);
        if (this._enabled) this.stop();
        else this.start();
        logInfo(`toggle() finished. New _enabled state is: ${this._enabled}`);
